...
function StatCounter({ value, label, color }) {
  ...
  return (
    <div ref={ref}>
      <div style={{ fontSize:'clamp(18px,3.5vw,30px)', fontWeight:800, fontFamily:"'Fraunces',serif", lineHeight:1.1, color: color || 'inherit' }}>{display}</div>
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:th.bg,color:th.color,overflowX:'hidden',transition:'background 0.4s,color 0.3s',position:'relative' }}>
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sec{padding:clamp(48px,7vw,112px) clamp(12px,4vw,40px)}
        ...
        @media(max-width:900px){
          .g2-about{grid-template-columns:1fr;gap:44px}
        }
        ...
        @media(max-width:768px){
          ...
          .sec{padding:clamp(40px,6vw,80px) clamp(12px,4vw,24px)}
        ...
        @media(max-width:600px){
          ...
          .g2-stats{grid-template-columns:1fr 1fr;gap:12px}
          .hero-cta{flex-direction:column;align-items:stretch}
          .hero-cta .btn-p,.hero-cta .btn-o{width:100%;justify-content:center;text-align:center}
          .trust-row{flex-direction:column;gap:8px!important}
        }

        /* ── RESPONSIVE: TINY (max 480px) ── */
        @media(max-width:480px){
          .btn-p,.btn-o{font-size:13px;padding:11px 20px}
          .g2-stats{grid-template-columns:1fr 1fr}
          .sec-label{font-size:9.5px;padding:5px 12px;letter-spacing:2px}
        }

        /* ── RESPONSIVE: EXTRA TINY (max 360px) ── */
        @media(max-width:360px){
          .g2-stats{grid-template-columns:1fr}
          .btn-p,.btn-o{font-size:12px;padding:10px 16px}
        }
      ...
      {/* ── NAV ──────────────────────────────────────────────────────────── */}
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:100,height:68,padding:'0 clamp(12px,4vw,40px)',display:'flex',alignItems:'center',justifyContent:'space-between',
        ...
        >
          ...
          <span style={{ fontSize:'clamp(16px,3vw,20px)',fontWeight:800,fontFamily:"'Fraunces',serif",letterSpacing:'-0.6px',color:th.color }}>
      ...
      {/* ── HERO ─────────────────────────────────────────────────────────── */}
      <section id="hero" style={{ minHeight:'100vh',display:'flex',alignItems:'center',position:'relative',overflow:'hidden',padding:'clamp(100px,14vw,148px) clamp(12px,4vw,40px) clamp(64px,8vw,100px)' }}>
        {/* Orbs — clamped to prevent horizontal overflow on mobile */}
        <div className="orb" style={{ width:'clamp(280px,65vw,720px)',height:'clamp(280px,65vw,720px)',background:'radial-gradient(circle,#0ea5e9 0%,#6366f1 60%)',top:-160,right:-160,opacity:isLight?0.07:0.14,transform:parallax(-0.07) }}/>
        <div className="orb" style={{ width:'clamp(200px,50vw,520px)',height:'clamp(200px,50vw,520px)',background:'radial-gradient(circle,#34d399,#0ea5e9)',bottom:-90,left:-130,opacity:isLight?0.06:0.11,transform:parallax(0.05) }}/>
        <div className="orb" style={{ width:'clamp(150px,30vw,320px)',height:'clamp(150px,30vw,320px)',background:'radial-gradient(circle,#8b5cf6,#ec4899)',top:'38%',right:'22%',opacity:isLight?0.05:0.1,transform:parallax(-0.04) }}/>
        ...
        <div style={{ maxWidth:700,position:'relative',zIndex:4,width:'100%' }}>
          ...
          {/* Hero heading — clamped letter-spacing for mobile */}
          <h1 className="fu d1" style={{
            fontFamily:"'Fraunces',serif",
            fontSize:'clamp(34px,7.5vw,92px)',
            fontWeight:900,
            lineHeight:1.04,
            letterSpacing:'clamp(-0.5px,-0.4vw,-3px)',
            margin:'0 0 24px',
            color:th.color
          }}>
            {t.heroTitle1}<br/>
          ...
          <p className="fu d2" style={{ fontSize:'clamp(14px,2.5vw,18px)',lineHeight:1.82,color:th.body,maxWidth:540,marginBottom:38 }}>
            {t.heroSub}
          </p>
          {/* CTA buttons — stack on small screens via .hero-cta class */}
          <div className="fu d3 hero-cta" style={{ display:'flex',gap:14,flexWrap:'wrap',marginBottom:58 }}>
          ...
          {/* Stats row — wraps to 2×2 on small screens */}
          <div className="fu d4" style={{ display:'flex',gap:'clamp(14px,4vw,52px)',flexWrap:'wrap',paddingTop:36,borderTop:`1px solid ${isLight?'rgba(14,165,233,0.15)':'rgba(14,165,233,0.12)'}` }}>
            {STATS.map((s,i)=>(
              <div key={i} style={{ minWidth:'clamp(72px,30%,120px)' }}>
          ...
          {/* Trust row */}
          <div className="fu d5 trust-row" style={{ display:'flex',alignItems:'center',gap:20,marginTop:28,flexWrap:'wrap' }}>
      ...
      <section id="courses" className="sec" style={{ maxWidth:1240,margin:'0 auto',position:'relative' }}>
        <div className="orb" style={{ width:'clamp(180px,35vw,380px)',height:'clamp(180px,35vw,380px)',background:'#0ea5e9',top:-80,right:-80,opacity:isLight?0.05:0.09 }}/>
        <SectionHeader label={t.coursesLabel} title1={t.coursesTitle1} title2={t.coursesTitle2} sub={t.coursesSub}/>
        <div className="g3">
          {COURSES.map((c,i)=>(
            <Reveal key={i} delay={i*0.07}>
              <div className="card" style={{ padding:'clamp(18px,3vw,34px)',height:'100%' }}>
                ...
                <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:20,gap:8 }}>
                  <div style={{ width:54,height:54,borderRadius:15,background:`${c.color}18`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:26,boxShadow:`0 4px 18px ${c.color}28`,transition:'transform 0.2s',cursor:'default',flexShrink:0 }}
                    onMouseEnter={e=>e.currentTarget.style.transform='scale(1.08) rotate(-5deg)'}
                    onMouseLeave={e=>e.currentTarget.style.transform='none'}
                  >{c.icon}</div>
                  <span style={{ fontSize:10,fontWeight:700,color:c.color,background:`${c.color}15`,border:`1px solid ${c.color}33`,borderRadius:100,padding:'4px 10px',letterSpacing:'0.5px',whiteSpace:'nowrap',flexShrink:0 }}>{c.badge}</span>
                </div>
                <h3 style={{ fontSize:'clamp(14px,2vw,17.5px)',fontWeight:700,marginBottom:10,color:th.color }}>{c.title[lang]}</h3>
                <p style={{ fontSize:13,color:th.sub,lineHeight:1.78,marginBottom:22 }}>{c.desc[lang]}</p>
      ...
      <section id="about" style={{ background:th.secBg,borderTop:`1px solid ${th.secBorder}`,borderBottom:`1px solid ${th.secBorder}`,position:'relative',overflow:'hidden' }}>
        <div className="orb" style={{ width:'clamp(200px,45vw,480px)',height:'clamp(200px,45vw,480px)',background:'#6366f1',top:-100,left:-100,opacity:isLight?0.05:0.1 }}/>
            ...
            {STATS.map((s,i)=>(
              <Reveal key={i} delay={i*0.09}>
                <div style={{
                  ...
                  borderRadius:20,padding:'clamp(16px,3vw,34px)',textAlign:'center',
      ...
      <section id="pricing" className="sec" style={{ maxWidth:1120,margin:'0 auto',position:'relative' }}>
        <div className="orb" style={{ width:'clamp(180px,40vw,420px)',height:'clamp(180px,40vw,420px)',background:'#34d399',bottom:-100,left:-100,opacity:isLight?0.05:0.09 }}/>
        <SectionHeader label={t.pricingLabel} title1={t.pricingTitle1} title2={t.pricingTitle2} sub={t.pricingSub}/>
        <div className="g3">
          {PRICING.map((p,i)=>(
            <Reveal key={i} delay={i*0.1}>
              <div className={`p-card${p.highlight?' hl':''}`} style={{ padding:'clamp(22px,3vw,42px)',height:'100%' }}>
                ...
                {/* Plan header row — badge is inline to avoid overflow on narrow cards */}
                <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8,marginBottom:10 }}>
                  <div style={{ fontSize:11,color:th.sub,fontWeight:700,letterSpacing:'0.5px',textTransform:'uppercase' }}>{p.name[lang]}</div>
                  {p.highlight && (
                    <div style={{ fontSize:9,fontWeight:700,letterSpacing:1.4,background:'linear-gradient(135deg,#0ea5e9,#6366f1)',color:'#fff',padding:'4px 10px',borderRadius:100,textTransform:'uppercase',boxShadow:'0 4px 16px rgba(14,165,233,0.35)',whiteSpace:'nowrap',flexShrink:0 }}>{t.pricingPopular}</div>
                  )}
                </div>
                <div style={{ display:'flex',alignItems:'baseline',gap:4,marginBottom:7 }}>
                  <span style={{ fontFamily:"'Fraunces',serif",fontSize:'clamp(24px,5vw,48px)',fontWeight:800,color:th.color,lineHeight:1.1 }}>{p.price}</span>
                ...
                <div style={{ marginBottom:34,display:'flex',flexDirection:'column',gap:12 }}>
                  {p.features[lang].map((f,j)=>(
                    <div key={j} style={{ display:'flex',alignItems:'flex-start',gap:10,fontSize:13,color:th.body,lineHeight:1.5 }}>
      ...
      <section id="testimonials" style={{ background:th.secBg,borderTop:`1px solid ${th.secBorder}`,position:'relative',overflow:'hidden' }}>
        <div className="orb" style={{ width:'clamp(160px,35vw,380px)',height:'clamp(160px,35vw,380px)',background:'#ec4899',top:-60,right:-60,opacity:isLight?0.05:0.09 }}/>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(260px,100%),1fr))',gap:22 }}>
            {TESTIMONIALS.map((tm,i)=>(
              <Reveal key={i} delay={i*0.08}>
                <div className="t-card" style={{ padding:'clamp(18px,3vw,34px)',height:'100%' }}>
                  ...
                  <p style={{ fontSize:13.5,lineHeight:1.82,color:th.body,marginBottom:22,fontStyle:'italic' }}>{tm.text[lang]}</p>
      ...
      <section id="contact" className="sec" style={{ maxWidth:800,margin:'0 auto',position:'relative' }}>
        <div className="orb" style={{ width:'clamp(150px,35vw,340px)',height:'clamp(150px,35vw,340px)',background:'#0ea5e9',top:-80,right:-80,opacity:isLight?0.06:0.11 }}/>
        <SectionHeader label={t.contactLabel} title1={t.contactTitle1} title2={t.contactTitle2} sub={t.contactSub}/>
        {submitted ? (
          <Reveal>
            <div style={{ textAlign:'center',padding:'clamp(36px,7vw,68px) clamp(20px,5vw,52px)',background:th.glass,border:'1px solid rgba(14,165,233,0.28)',borderRadius:26,backdropFilter:'blur(18px)' }}>
              <div style={{ fontSize:58,marginBottom:18,animation:'scaleIn 0.4s cubic-bezier(.34,1.56,.64,1) both',display:'block' }}>🎉</div>
              <h3 style={{ fontSize:'clamp(18px,3.5vw,28px)',fontWeight:800,marginBottom:12,fontFamily:"'Fraunces',serif",color:th.color }}>{t.successTitle}</h3>
              <p style={{ color:th.sub,fontSize:15.5,lineHeight:1.75 }}>{t.successSub}</p>
            </div>
          </Reveal>
        ) : (
          <Reveal>
            <form onSubmit={handleSubmit} style={{ background:th.glass,border:`1px solid ${th.cardBorder}`,borderRadius:26,padding:'clamp(24px,5vw,54px)',display:'flex',flexDirection:'column',gap:20,backdropFilter:'blur(18px)' }}>
      ...
      {/* ── FOOTER ───────────────────────────────────────────────────────── */}
      <footer style={{ borderTop:`1px solid ${th.secBorder}`,padding:'clamp(20px,4vw,42px) clamp(12px,4vw,40px)',position:'relative',background:isLight?'rgba(240,249,255,0.75)':'rgba(4,9,15,0.75)',backdropFilter:'blur(14px)' }}>
